...
REM Name of the temporary file that holds ping info
	set TEMPDIR=temp.ping

:: Skip to the first ping if called from parent thread
if /I "%~1" == "pingaddressinit" goto pingaddressinit
...
REM Values used for basic functionality
	...
	:: Whether or not to spawn child ping process
	set FIRSTPING=!TRUE!
...
:eventloop
	...
	if "!PROGRAMSTATE!" == "RUN" (
		...
		if "!DISPLAYSTATE!" == "init" (
			...
			if "!STARTPING!" == "!TRUE!" (
				:: Releases the first ping to a separate thread to continue animating the initialization screen
				if "!FIRSTPING!" == "!TRUE!" (
					start /b "" cmd /c "%~0" "pingaddressinit" "!IPADDR!"
					set FIRSTPING=!FALSE!
				)

				:: Go to the menu once the first ping has completed
				:: credit - https://stackoverflow.com/questions/10518151/how-to-check-in-command-line-if-a-given-file-or-directory-is-locked-used-by-any
				2>nul (
					>>!TEMPDIR! (call )
			...
			if "!DISPLAYSTATE!" == "menu" (
				rem The program now displays a menu to the screen
			) else (
				if "!DISPLAYSTATE!" == "exec" (
					rem The program no starts executing
				) else (
					if "!DISPLAYSTATE!" == "opts" (
						rem the program now gives you options of sorts
					) else (
						if "!DISPLAYSTATE!" == "help" (
							rem the program gives you a helpful message
						) else (
		...
		:: Update the display
		rem call :updateuserinterface "!DISPLAYMSG!" !DISPLAYMSG_ROW! !DISPLAYMSG_COL! !TRUE!
	) else (
		...
		if !_actionafterdisplay! == "%THENPAUSEPROMPT%" (
			...
			if !_actionafterdisplay! == "%THENPAUSENOPROMPT%" (
				...
				if !_actionafterdisplay! == "%THENWAITPROMPT%" (
					...
					REM Treat any other command as "do nothing," which does not interrupt the program
				)
			)
		)
	)
...
:setwindowsizes
	...
	rem mode !WIDTH!,!HEIGHT!

	set _RETURN=
	goto :EOF

:: Subroutine that pings a given address in a separate thread
:: Arguments: ipAddress
:pingaddressinit
	ping -n !SAMPLESIZE! %~2 > !TEMPDIR! 2>&1
